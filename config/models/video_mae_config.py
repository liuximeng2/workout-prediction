"""VideoMAE-specific configuration."""
...
@dataclass
class VideoMAEConfig(BaseConfig):
    """Fine-tuning configuration for VideoMAE.

    Inherits all global settings from :class:`~config.base_config.BaseConfig`
    and adds VideoMAE-specific knobs.
    """

    # ── Model ────────────────────────────────────────────────────────────────
    model_name: str = "video_mae"
    model_ckpt: str = "MCG-NJU/videomae-base"

    # Checkpoint directory (relative to repo root)
    output_dir: str = "checkpoints/videomae-workout"
